	...
	if texture then
		if image then
			...
			if not iconStack then
				ContextPtr:BuildInstanceForControlAtIndex( "Stack0", anchor, anchor.Stack, 0 )
				iconStack = anchor.Stack0
			end
			ContextPtr:BuildInstanceForControlAtIndex( "Improvement", anchor, iconStack, 0 )
...
local function UpdateResource( anchor, plot )
	local iconStack = anchor.Stack0
	local image = anchor.ResourceImage
	...
	if texture then
		if image then
			...
			ContextPtr:BuildInstanceForControl( "Resource", anchor, iconStack )
...
local function UpdateYields( anchor, plot )
	...
	repeat
		for i = i, j do
			...
			yieldIcon = anchor[k]
			if yieldIcon then
				yieldIcon:ChangeParent( iconStack )
			else
				...
				if yieldIcon then
					...
					yieldIcon = {}
					ContextPtr:BuildInstanceForControl( "Image", yieldIcon, iconStack )
					yieldIcon = yieldIcon.Image
				end
				anchor[k] = yieldIcon
			end
			k = k + 1
			...
			if amount > 5 then
				...
				if amountIcon then
					...
					if amountIcon then
						...
						amountIcon = {}
						ContextPtr:BuildInstanceForControl( "Image", amountIcon, yieldIcon )
						amountIcon = amountIcon.Image
					end
				end
				anchor[k] = amountIcon
				k = k + 1
		...
		if j < n then
			i = j + 1
			j = n
			iconStack = anchor.Stack2
			if not iconStack then
				ContextPtr:BuildInstanceForControl( "Stack2", anchor, anchor.Stack )
				iconStack = anchor.Stack2
			end
		else
			break
